...
const fulfillOrder = async (_network, _userAddress, _order) => {
  try {
    if (!_order || _order.length === 0) throw new Error('No orders to fulfill')
    const { data } = await axios.post(`${env.SERVER_URL}/functions/opensea_fulfillOrder`, {
      network: _network,
